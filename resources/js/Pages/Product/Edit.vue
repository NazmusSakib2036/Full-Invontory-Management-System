...
onMounted(() => {
    ...
    if (props.product.photo) {
        if (props.product.photo.startsWith('http')) {
            previewImage.value = props.product.photo; // already full URL
        } else {
            previewImage.value = `/storage/${props.product.photo}`; // from storage/app/public
        }
    } else {
